...
from app.mqtt.handlers import log_device_telemetry
            ...
            async with aiomqtt.Client(
                ...
                async with client.messages() as messages:
                    async for message in messages:
                        ...
                        imei = topic.split("/")[-2] if "provision" in topic else topic.split("/")[-1]

                        print(f"[MQTT RX] {topic} -> {payload_str}")

                        await log_device_telemetry(imei, topic, message.payload)

                        with SessionLocal() as db:
                            try:
                                if topic.startswith("gas/status/"):
                                    await handlers.handle_status_message(payload_str, topic, db, client)
                                elif "provision" in topic:
                                    await handlers.handle_provision_request(payload_str, topic, db, client)
