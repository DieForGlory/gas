...
export const getDeviceTelemetry = async (imei, skip = 0, limit = 50) => {
    const response = await api.get(`/devices/${imei}/telemetry`, {
        params: { skip, limit }
    });
...
export const firmwareService = {
    ...
    triggerOTA: (firmware_id, imeis) => api.post('/firmware/bulk-ota', { firmware_id, imeis })
};
...
export const authService = {
  login: (credentials) => {
    ...
    params.append('username', credentials.username.trim().toLowerCase());
