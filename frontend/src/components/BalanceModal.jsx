...
const BalanceModal = ({ subscriber, onClose, onUpdate }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      alert(error.response?.data?.detail || t('balance_error', 'Ошибка при пополнении баланса'));
    } finally {
      setLoading(false);
    }
  };
        ...
        {!success ? (
          <>
            <div className="p-8 pb-4 flex justify-between items-start">
              <div>
                <h3 className="text-2xl font-black text-slate-900 tracking-tight">{t('top_up', 'Пополнение')}</h3>
                <p className="text-slate-500 text-sm font-medium mt-1">{t('account_abbr', 'Л/С')}: {subscriber.account_number}</p>
              </div>
            ...
            <form onSubmit={handleSubmit} className="p-8 pt-4 space-y-6">
              <div className="bg-slate-50 rounded-2xl p-6 border border-slate-100 shadow-inner">
                <div className="flex justify-between items-center mb-4">
                  <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">{t('current_balance', 'Текущий баланс')}</span>
              ...
              <div className="space-y-3">
                <label className="text-[11px] font-bold text-slate-400 uppercase tracking-widest px-1">{t('top_up_amount', 'Сумма пополнения')}</label>
              ...
              >
                {loading ? t('processing', 'ОБРАБОТКА...') : (
                  <>{t('pay_button', 'ПОПОЛНИТЬ СЧЕТ')} <ChevronRight size={20} /></>
                )}
              </button>
            </form>
          </>
          ...
          <div className="p-12 text-center animate-in zoom-in duration-300">
            ...
            <h3 className="text-2xl font-black text-slate-900 mb-2">{t('success', 'Успешно!')}</h3>
            <p className="text-slate-500 font-medium italic">{t('balance_updated', 'Баланс абонента обновлен')}</p>
          </div>
        )}
      </div>
    </div>
