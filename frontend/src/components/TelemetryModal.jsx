        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg shadow-xl w-full max-w-4xl max-h-[90vh] flex flex-col">
                <div className="flex justify-between items-center p-4 border-b">
                    <h2 className="text-lg font-bold">История телеметрии: {imei}</h2>
                    <button onClick={onClose} className="text-gray-500 hover:text-gray-700">Закрыть</button>
                </div>
                <div className="overflow-auto p-4 flex-grow bg-gray-50">
                    {logs.length === 0 && !loading ? (
                        <p className="text-gray-500">Нет данных от устройства.</p>
                    ) : (
